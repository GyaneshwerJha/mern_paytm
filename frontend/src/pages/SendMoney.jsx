  ...
  return (
    <div className="flex justify-center h-screen items-center bg-gray-200">
      <div className="p-8 flex flex-col border shadow-lg md:w-2/5 max-w-96 bg-white rounded-3xl">
        <h2 className="text-bold mb-3 text-2xl md:text-3xl md:mb-5 m-auto drop-shadow-xl p-1">
          Send Money
        </h2>
        <div className="flex items-center">
          <svg
            ...
            strokeWidth="1.5"
            stroke="black"
            className="border-black w-7 h-7 mr-2 border-2 rounded-full p-1"
          >
            <path
              strokeLinecap="round"
        ...
        <input
          ...
          className="border border-inherit mb-3 p-2"
          type="number"
          placeholder="Enter Amount"
        />
        <button
          onClick={handleTransaction}
          className="bg-teal-500 rounded-md drop-shadow p-2 hover:bg-teal-600"
        >
          Initiate Transfer
        </button>
      </div>
      {showLoader && (
        <div className="absolute w-full h-full flex flex-col items-center justify-center bg-gray-200 opacity-75">
          <Loader></Loader>
